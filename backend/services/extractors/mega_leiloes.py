...
class MegaLeiloesExtractor(BaseExtractor):
    ...
    def extract(self, soup=None, url=None) -> Dict[str, Any]:
        ...
        try:
            ...
            data = {
                ...
                "images": images,
                "documents": [],  # Implementar extração
    ...
    def _extract_title(self) -> str:
        title_tag = self.soup.find('h1', class_='title')
        if not title_tag:
    ...
    def _extract_location(self) -> Tuple[str, str, str]:
        ...
        state = ""
        # Endereço
        address_tag = self.soup.find(lambda tag: tag.name in ['div', 'p'] and tag.get('class') and any('address' in c.lower() for c in tag.get('class')))
        if address_tag:
